...
export async function uploadInvoicePDF(
  ...
  // Use upload_stream instead of base64 data-URI:
  //   - avoids 33% payload inflation from base64 encoding
  //   - skips the full buffer→string allocation
  //   - streams bytes directly to Cloudinary's upload endpoint
      ...
      const stream = cl.uploader.upload_stream(
        ...
        (error, uploadResult) => {
          if (error || !uploadResult) {
            return reject(error ?? new Error('[uploadInvoicePDF] No result returned from Cloudinary'))
          }
