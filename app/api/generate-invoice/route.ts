...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const invoiceDownloadUrl = `${baseUrl}/api/view-invoice?url=${encodedUrl}&dl=1`
        ...
        // ── Send email + WhatsApp in parallel, awaited before returning ──────
    //
    // Previously both were fire-and-forget. Vercel freezes this function the
    // moment `return NextResponse.json(...)` executes, killing any in-flight
    // fetch that hasn't completed — which is why emails were silently dropped.
    // Running them in parallel with Promise.allSettled keeps the function alive
    // until both complete (or fail), without adding serial latency.
          ...
          () => fetch(`${baseUrl}/api/send-email`, {
            ...
            body: JSON.stringify({
              ...
              data: {
                ...
                html:      buildInvoiceHTML(order as never, customerName, customerEmail),
              },
            }),
          }),
          3, 500,
        ).then(() => console.log(`[generate-invoice] ✅ Email sent → ${customerEmail}`))
    ...
    const whatsappPromise = !effectivePhone
      ? Promise.resolve(
          console.error(
      ...
      : !whatsappOptIn
        ...
        : fetchWithRetry(
            ...
            3, 500,
          ).then(() => console.log(`[generate-invoice] ✅ WhatsApp invoice triggered → ${effectivePhone}`))
            .catch(err =>
              console.error(`[generate-invoice] ❌ WhatsApp failed after retries for ${order_number}:`, err),
            )

    // Wait for both — failures are already logged above, allSettled never throws
